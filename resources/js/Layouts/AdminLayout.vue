...
const navLinks = [
  ...
  { name: 'Work Experience',  href: route('admin.work-experiences.index'), icon: 'briefcase' },
...
<template>
  <div class="min-h-screen flex bg-background">
    ...
    <!-- ── Sidebar ── -->
    <aside class="w-60 flex-shrink-0 flex flex-col bg-foreground min-h-screen">

      <!-- Brand -->
      <div class="px-5 py-5 border-b border-white/10">
        <Link :href="route('home')" class="block">
          <div class="text-white font-display font-bold text-base leading-tight">Belinze</div>
          <div class="text-xs mt-0.5 text-primary font-medium">Admin Panel</div>
        </Link>
      </div>
      ...
      <div class="px-3 py-4 border-t border-white/10 space-y-0.5">
        <Link
          :href="route('home')"
          class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-white/50 hover:text-white hover:bg-white/8 transition-all"
        >
        ...
        <button
          @click="logout"
          class="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-white/50 hover:text-red-400 hover:bg-white/8 transition-all text-left"
        >
          <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <header class="bg-card border-b border-border px-6 py-3 flex items-center justify-between">
        <h1 class="text-sm font-medium text-muted-foreground font-display">Admin Panel</h1>
      ...
      <!-- Flash messages -->
      <div v-if="flash.success" class="mx-6 mt-4 px-4 py-3 rounded-xl text-sm font-medium text-green-700 bg-green-50 border border-green-200 flex items-center gap-2">
        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
        </svg>
        {{ flash.success }}
      </div>
      <div v-if="flash.error" class="mx-6 mt-4 px-4 py-3 rounded-xl text-sm font-medium text-red-700 bg-red-50 border border-red-200 flex items-center gap-2">
        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
